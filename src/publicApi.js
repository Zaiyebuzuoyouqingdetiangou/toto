import { createRabbitMirrorPublicAPI } from './publicApiCore.js?rmv=1.6.16-test.1';
import { getSettings, updateSettings } from './settings.js?rmv=1.6.16-test.1';
...
export function installRabbitMirrorPublicAPI(isActive, target = globalThis) {
    const connection = () => import('./independentApi/connection.js?rmv=1.6.16-test.1');
    ...
    const handle = createRabbitMirrorPublicAPI({
        ...
        async complete(settings, system, prompt, options) {
            const runtime = await import('./independentApi/request.js?rmv=1.6.16-test.1');
