import { createRabbitMirrorPublicAPI } from './publicApiCore.js?rmv=1.6.16-test.8';
...
export function installRabbitMirrorPublicAPI(isActive, target = globalThis) {
    const connection = () => import('./independentApi/connection.js?rmv=1.6.16-test.8');
    ...
    const handle = createRabbitMirrorPublicAPI({
        ...
        async complete(settings, system, prompt, options) {
            const runtime = await import('./independentApi/request.js?rmv=1.6.16-test.8');
