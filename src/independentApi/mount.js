...
import { presentationModeFields, isBlankLongTextSelection } from '../presentationMode.js?rmv=1.6.4-longtext2';
...
} from '../outputSanitizer.js?rmv=1.6.4-longtext2';
...
} from '../visualScanner.js?rmv=1.6.4-longtext2';
...
import { getRabbitMirrorRecipe, recordRabbitMirrorRecipe } from '../blacklist.js?rmv=1.6.4-longtext2';
...
import {
    ...
    recipesCoverMissing,
} from '../missingFaceMerge.js?rmv=1.6.4-longtext2';
import {
...
} from './flights.js?rmv=1.6.4-longtext2';
import {
    HISTORY_PANEL_ATTR,
    ...
    writePersistedOwner,
    writeStore,
} from './persistence.js?rmv=1.6.4-longtext2';
import {
...
} from './faceSwipe.js?rmv=1.6.4-longtext2';
import {
...
} from './request.js?rmv=1.6.4-longtext2';
import {
...
} from './lifecycle.js?rmv=1.6.4-longtext2';
   ...
   if(force && previousReadyRecord?.html){
    ...
    if(currentHost){
     ...
     showEphemeralFaceFailure(currentHost,faceIndex,failureMessage,
      ()=>{ void import('../outputSanitizer/toolsChrome.js?rmv=1.6.4-longtext2').then(module=>module.openRabbitMirrorResayChooser(overlayRoot)).catch(()=>globalThis.toastr?.warning?.('重说面板未能打开，请从工具菜单重试。')); },
...
export function resayIndependentMirror(root,owner={},options={}){
 ...
 if(getSettings().generationSource==='follow'){
  ...
  void import('../followFaceRetry.js?rmv=1.6.4-longtext2').then(({retryFollowFace})=>retryFollowFace(root,owner,{
...
async function requestMirrorImagePlan(target,input={},options={}){
 ...
 const {buildImagePlanningPrompt,parseImagePlan}=await import('../imagePlan.js?rmv=1.6.4-longtext2');
 target.assertCurrent();
...
function handleFollowMultifaceRejected(event){
 ...
 queueMessageSync([index]);
 void import('../followAutomaticReroll.js?rmv=1.6.4-longtext2').then(({maybeAutomaticFollowReroll})=>maybeAutomaticFollowReroll(index,followAutomaticRerollDeps()));
 return true;
}
...
function handleFollowGenerationSettled(event){
 ...
 if(event?.detail?.cancelled===true){
  void import('../followAutomaticReroll.js?rmv=1.6.4-longtext2').then(({markFollowAutomaticRerollCancelled})=>{
