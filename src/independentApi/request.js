...
import { presentationModeFields, hasExplicitTextFace } from '../presentationMode.js?rmv=1.6.16-test.2';
...
} from '../promptBuilder.js?rmv=1.6.16-test.2';
...
import { cleanRabbitMirrorOutput } from '../outputSanitizer.js?rmv=1.6.16-test.2';
...
import { recordRabbitMirrorIndependentPrompt } from '../tokenMeter.js?rmv=1.6.16-test.2';
...
import { operationEpochForBase } from './flights.js?rmv=1.6.16-test.2';
import {
...
import { assertEarlyBodyOwner } from './earlyBody.js?rmv=1.6.16-test.2';
 ...
 for(const details of externalFaceDetails(host)){
  ...
  if(resay && !independentRejectedFaceControlsWired.has(resay)){
   ...
   resay.addEventListener('click',event=>{
    event.preventDefault(); event.stopPropagation();
    void import('../outputSanitizer/toolsChrome.js?rmv=1.6.16-test.2').then(module=>module.openRabbitMirrorResayChooser(details)).catch(()=>globalThis.toastr?.warning?.('重说面板未能打开，请从工具菜单重试。'));
   },true);
  }
 }
}
