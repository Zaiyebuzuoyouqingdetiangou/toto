...
${presentationMode === 'longtext'
    ...
    : formats.length
        ? '依据 formats 与本面成品共同确定实际媒介和构图，将所选展现形式本身画出来。例如信件应呈现有本面内容依据的信件，相册应呈现对应的相册组织与照片内容，印象盒应呈现承载本面印象的物件及其组织关系；这些只是对应示例，不是固定模板。不要一律改画成“人物拿着它”或通用人物场景。多个形式以成品实际采用的结构为准，不为逐项照搬名称而硬塞拼盘。'
        : '本面没有可信展现形式记录：只从成品实际内容选择有依据的可见画面；不要仅凭标题猜造一个抽取形式，也不为了形式落地而额外发明信件、相册、盒子或其他容器。'}
不预设人物必须出镜，不预设固定容器、固定版式或固定插画模板。只有材料和当前构图需要的人物、物件、空间才进入画面；文字和物件不添加未发生的情节或未确认的身份。`;
