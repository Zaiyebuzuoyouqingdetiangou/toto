import { getSettings } from './settings.js?rmv=1.2.9.2';
import { getCurrentChatKey } from './storage.js?rmv=1.2.9.2';
import {
    FEEDBACK_CAT_TYPES,
...
} from './feedbackCat.js?rmv=1.2.9.2';
import { scanRabbitMirrorHtml } from './visualScanner.js?rmv=1.2.9.2';
import { getRabbitMirrorGenerationSnapshot } from './generationGuard.js?rmv=1.2.9.2';


const RUNTIME_VERSION = '1.2.9.2';
...
// 1.2.9.2: SillyTavern 的 Markdown/正则渲染链有时会把本应独立的兔子镜
// DOM 错误留在上一块“状态栏/行动选项/其他模块”的容器内部。结果是兔子镜继承
// 对方的宽度、overflow、定位和显隐规则，看起来像被另一模块吞进去。
// 这里只做高置信的 DOM 重新挂载：不改兔子镜 HTML/CSS、配色、Prompt 或内容。
const RABBIT_MIRROR_FOREIGN_WRAPPER_REHOME_ATTR = 'data-rabbit-mirror-rehomed-from-foreign-wrapper';

function rabbitMirrorForeignWrapperSignature(element) {
    if (!element) return '';
    const id = String(element.id || '').trim();
    const classes = [...(element.classList || [])].slice(0, 6).join('.');
    return `${String(element.tagName || '').toLowerCase()}${id ? `#${id}` : ''}${classes ? `.${classes}` : ''}`.slice(0, 180);
}

function rabbitMirrorForeignWrapperEvidence(root, body, ancestors = []) {
    if (!root || !body) return { strong: false, reason: '' };

    // <toto> 本身就是兔子镜协议的最外层。只要它还被塞在 .mes_text 内的另一层
    // DOM 下面，这一层就不是兔子镜自身结构，可以直接视作高置信错误包裹。
    if (root.matches?.(MIRROR_TOTO_SELECTOR)) return { strong: true, reason: 'toto-nested-under-message-wrapper' };

    const semanticForeign = ancestors.some(element => {
        if (!element?.matches) return false;
        if (element.matches(MIRROR_TOTO_SELECTOR)) return false;
        if (element.matches?.('details') && isRabbitMirrorDetails(element)) return false;
        if (element.matches('details, form, fieldset, label, nav, aside, table, [role="dialog"], [role="menu"], [role="tablist"], [role="listbox"], [role="group"]')) return true;
        const hint = `${element.id || ''} ${[...(element.classList || [])].join(' ')}`.toLowerCase();
        return /(?:status|state|option|action|choice|select|module|widget|panel|card|sheet|drawer|menu|toolbar|control|profile|attribute|stats|sidebar|accordion)/i.test(hint);
    });
    if (semanticForeign) return { strong: true, reason: 'foreign-module-ancestor' };

    // 被包裹时，兔子镜所在分支周围通常还残留上一模块自己的正文/按钮。
    let surroundingText = 0;
    let branch = root;
    for (const ancestor of ancestors) {
        for (const sibling of [...(ancestor.children || [])]) {
            if (sibling === branch || ['STYLE', 'SCRIPT', 'TEMPLATE', 'LINK', 'META'].includes(sibling.tagName)) continue;
            surroundingText += String(sibling.textContent || '').replace(/\s+/g, '').length;
            if (surroundingText >= 28) return { strong: true, reason: 'foreign-sibling-content' };
        }
        branch = ancestor;
    }

    // <toto> 被宿主剥掉时只剩 details，若它因为上层容器明显变窄，也可作为补充证据。
    try {
        const rootRect = root.getBoundingClientRect?.();
        const bodyRect = body.getBoundingClientRect?.();
        const rootWidth = Number(rootRect?.width || 0);
        const bodyWidth = Number(bodyRect?.width || 0);
        if (bodyWidth >= 260 && rootWidth >= 80 && rootWidth / bodyWidth <= 0.84) {
            return { strong: true, reason: 'foreign-wrapper-width-trap' };
        }
    } catch {}
...
function rehomeRabbitMirrorFromForeignWrapper(root) {
    if (!root?.isConnected || !root.closest) return false;
    if (root.closest('[data-rabbit-mirror-external-source], [data-rabbit-mirror-external-shell]')) return false;
    const body = root.closest('.mes_text');
    if (!body || root.parentElement === body) return false;

    const ancestors = [];
    let current = root.parentElement;
    while (current && current !== body) {
        if (current.matches?.(MIRROR_TOTO_SELECTOR) || (current.matches?.('details') && isRabbitMirrorDetails(current))) return false;
        if (current.closest?.('[data-rabbit-mirror-tool-entry-host]')) return false;
        ancestors.push(current);
        current = current.parentElement;
    }
    if (current !== body || !ancestors.length) return false;

    const evidence = rabbitMirrorForeignWrapperEvidence(root, body, ancestors);
    if (!evidence.strong) return false;

    // ancestors 最后一项就是 .mes_text 的直接子元素。兔子镜从该模块内部抽离后，
    // 紧跟在该模块之后，保持消息中的先后顺序，而不是统一扔到消息末尾。
    const carrier = ancestors[ancestors.length - 1];
    if (!carrier?.parentElement || carrier.parentElement !== body) return false;
    const signature = rabbitMirrorForeignWrapperSignature(carrier);
    carrier.insertAdjacentElement('afterend', root);
    root.setAttribute(RABBIT_MIRROR_FOREIGN_WRAPPER_REHOME_ATTR, 'true');
    root.dataset.rabbitMirrorForeignWrapperReason = evidence.reason;
    if (signature) root.dataset.rabbitMirrorForeignWrapperFrom = signature;
    return true;
}

function rehomeRabbitMirrorsFromForeignWrappersInScope(scope) {
    if (!scope?.querySelectorAll) return 0;
    let changed = 0;
    // 先拍快照再移动，避免 querySelectorAll 的 live 结构变化影响遍历。
    for (const root of [...getRenderedRabbitMirrorInteractionRoots(scope)]) {
        try { if (rehomeRabbitMirrorFromForeignWrapper(root)) changed += 1; }
        catch (error) { console.debug('[RabbitMirror] foreign wrapper rehome skipped for one mirror:', error); }
    }
    return changed;
}
...
const INTERACTION_DIAGNOSTIC_VERSION = '1.2.9.2-FULL-CHAIN';
...
function installMaintenanceRabbitsInScope(scope, { allowGlobalRemoval = false } = {}) {
    ...
    // 先把被上一块模块错误包裹的兔子镜抽回消息正文层，再安装维修/交互工具。
    // 该步骤只移动现有 DOM 节点，不重写生成内容与任何审美样式。
    rehomeRabbitMirrorsFromForeignWrappersInScope(scope);
