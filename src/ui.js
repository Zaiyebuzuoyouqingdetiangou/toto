import { DEFAULT_VISUAL_PROMPT, VISUAL_AVOID_PROMPT_MAX_CHARS, VISUAL_EXTRA_PROMPT_MAX_CHARS, VISUAL_PROMPT_MAX_CHARS, getSettings, updateSettings, resetSettings } from './settings.js?rmv=1.4.2';
import { clearLastCombo } from './storage.js?rmv=1.4.2';
import { clearRabbitMirrorPrompt } from './injector.js?rmv=1.4.2';
import { clearFeedbackCatExtensionPrompt, getActiveFeedbackForCurrentChat, syncFeedbackCatExtensionPrompt } from './feedbackCat.js?rmv=1.4.2';
import { configureMaintenanceAutoSafeMode, refreshFeedbackCats, refreshMaintenanceRabbits, refreshRecipeButtons } from './outputSanitizer.js?rmv=1.4.2';
import { scanMemoryPlugins, testMemoryProvider } from './memoryScanner.js?rmv=1.4.2';
import { getLastRabbitMirrorTokenRecord, TOKEN_METER_EVENT } from './tokenMeter.js?rmv=1.4.2';
import { API_REQUEST_DIAGNOSTIC_EVENT, fetchIndependentModels, getLastIndependentApiRequestDiagnostic, refreshRabbitMirrorGenerationMode, testIndependentConnection } from './independentApi.js?rmv=1.4.2';
import { BLACKLIST_CHANGED_EVENT, blacklistEntries, blacklistPoolStats, clearBlacklist, removeBlacklistItem, setBlacklistEnabled } from './blacklist.js?rmv=1.4.2';

const SETTINGS_UI_VERSION = '1.4.2-visual-maintenance';
...
<div id="rabbit_mirror_theater_settings" class="rabbit-mirror-settings" data-rabbit-mirror-ui-version="${SETTINGS_UI_VERSION}" data-rabbit-mirror-runtime-version="${RUNTIME_VERSION}">
  <div class="inline-drawer">
    ...
    $('#rh_visual_prompt_reset').on('click', () => {
        ...
        renderVisualPromptStatus(getSettings());
        toastr?.success?.('已恢复默认通用视觉规则；额外视觉偏好与避雷内容保持不变。');
    });
