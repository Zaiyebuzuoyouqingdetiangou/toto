...
import { buildRabbitMirrorSettingsDialogHtml, buildWorldInfoPromptModalHtml, buildTagFilterModalHtml } from './ui/settingsTemplate.js?rmv=1.6.16-test.7';
import { attachIndependentApiDiagnosticListener, attachTokenMeterListener, renderIndependentApiDiagnostic, renderTokenMeter } from './ui/tokenMeter.js?rmv=1.6.16-test.7';
import { attachWorldInfoBooksListener, clearCollapsedAllWorldInfoBookRows, clearPulledWorldInfoBooks, installWorldInfoBookVisibilityObserver, pullAllWorldInfoBooks, renderWorldInfoBookSettings, resetWorldInfoBookUiState } from './ui/worldInfoBooks.js?rmv=1.6.16-test.7';
import { installTtDiagnosticEntry } from './ui/ttDiagnostics.js?rmv=1.6';

import { normalizePresentationModes } from './presentationMode.js?rmv=1.6.16-test.7';
...
import { clearRabbitMirrorPrompt, startManualEntryDiagnostic, stopManualEntryDiagnostic, getManualEntryDiagnosticState } from './injector.js?rmv=1.6.16-test.7';
...
import { BLACKLIST_CHANGED_EVENT, blacklistEntries, blacklistPoolStats, clearBlacklist, removeBlacklistItem, setBlacklistEnabled, favoriteEntries, removeFavoriteItem, setFavoriteMultiplier, clearFavorites } from './blacklist.js?rmv=1.6.16-test.7';
...
import { mountSettingsAppearance, destroySettingsAppearance } from './settingsAppearance.js?rmv=1.6.16-test.7';
...
export function initRabbitMirrorUI() {
    ...
    $('#rh_update_now').on('click', async event => {
        ...
        try {
            const updater = await import('./extensionUpdater.js?rmv=1.6.16-test.7-update1');
    ...
    const loadQuickStart = async () => {
        ...
        guideLoading = true;
        try {
            const module = await import('./quickStart.js?rmv=1.6.16-test.7');
    ...
    for (const [id, initialView] of Object.entries(libraryEntryViews)) {
        ...
        button.addEventListener('click', async () => {
            ...
            try {
                const module = await import('./externalWorldBook/importWizard.js?rmv=1.6.16-test.7');
    ...
    mountSettingsAppearance(document.getElementById('rabbit_mirror_theater_settings'), {
        onNavigate(page) {
            ...
            if (page === 'library') {
                void import('./externalWorldBook/importWizard.js?rmv=1.6.16-test.7').then(module => {
