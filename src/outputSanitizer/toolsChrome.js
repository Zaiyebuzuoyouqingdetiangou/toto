...
import { installMirrorToolMenu, fitMirrorToolPanel } from '../mirrorToolMenu.js?rmv=1.6.16-test.10';
import { placeFacePager } from '../facePagerPlacement.js?rmv=1.6.4-pager1';
import { isTextPresentation } from '../presentationMode.js?rmv=1.6.16-test.10';
import { isRabbitMirrorManagedChatSurface } from '../hostCompatibility.js?rmv=1.6.3-ttchild1';
import { getSettings, syncExternalReferenceVisibility } from '../settings.js?rmv=1.6.16-test.10';
import { getCurrentChatKey } from '../storage.js?rmv=1.6.16-test.10';
import { getSanitizedRabbitMirrorFaceProof } from '../multifaceProof.js?rmv=1.6.16-test.10';
import {
...
import { scanRabbitMirrorHtml } from '../visualScanner.js?rmv=1.6.16-test.10';
import {
    ...
    toggleBlacklistItem,
    toggleFavoriteItem,
} from '../blacklist.js?rmv=1.6.16-test.10';
import {
...
import { getAvailableHostChat } from './scriptedInteractionRescue.js?rmv=1.6.16-test.10';
import { armNestedDetailsReplacementContainment, installNestedDetailsReplacementContainment } from './fallbackRescue.js?rmv=1.6.16-test.10';
import { armRabbitMirrorFirstUseInteraction, repairRabbitMirrorScopedClassAliasesInScope } from './idsAndRearm.js?rmv=1.6.16-test.10';
import {
...
import { clearOrphanedStructuredStaticDisclosureArtifacts } from './choiceRescue.js?rmv=1.6.16-test.10';
import {
...
function loadMirrorImageModule() {
    if (!mirrorImageModule) mirrorImageModule = import('../imageUi.js?rmv=1.6.16-test.10').catch(error => { mirrorImageModule = null; throw error; });
...
function installUnifiedMirrorTools(root) {
    ...
    if (mirrorFaceCanContinue(root)) {
        actions.push({ id: 'continue', label: '✎ 续写', run: () => {
            void import('../independentApi/mount.js?rmv=1.6.16-test.10').then(module => module.continueIndependentLongText(root))
    ...
    actions.push({ id: 'theater-favorite-library', label: '📖 打开收藏夹', run: () => {
        void import('../independentApi.js?rmv=1.6.16-test.10').then(module =>
