...
import { MODULE_NAME, getSettings } from './settings.js?rmv=1.4.7';
import {
...
} from './feedbackCat.js?rmv=1.4.7';
import { recordRabbitMirrorInjection, recordRabbitMirrorNoInjection } from './tokenMeter.js?rmv=1.4.7';
...
function loadPromptBuilder() {
    if (!promptBuilderPromise) {
        promptBuilderPromise = import('./promptBuilder.js?rmv=1.4.7').catch(error => {
...
function loadGenerationGuard() {
    if (!generationGuardPromise) {
        generationGuardPromise = import('./generationGuard.js?rmv=1.4.7').catch(error => {
