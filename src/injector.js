...
import { MODULE_NAME, getSettings } from './settings.js?rmv=1.6.7';
import {
...
function loadPromptBuilder() {
    if (!promptBuilderPromise) {
        promptBuilderPromise = import('./promptBuilder.js?rmv=1.6.7').catch(error => {
...
function loadGenerationGuard() {
    if (!generationGuardPromise) {
        generationGuardPromise = import('./generationGuard.js?rmv=1.6.7').catch(error => {
