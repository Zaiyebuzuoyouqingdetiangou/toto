...
`;
    ...
    return String.raw`
【ID → 母本片段检索策略】balanced：完整四大原始规则母本保存在插件 data/raw/ 内，本轮不把完整大库全部塞进 prompt。插件先从结构化索引抽取 ID，再按 ID 到母本中检索对应完整描述，只注入本轮抽中的母本片段。
...
【本轮随机主题元素：必须使用 1-3 个并融合；以下为按 ID 从母本提取的对应完整描述】
${selectedThemes}
