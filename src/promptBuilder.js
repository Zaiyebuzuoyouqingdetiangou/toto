...
【兔子洞副 API 模式】
本轮不要生成兔子洞、小剧场、<toto> 或 <details> 模块。
...
6. 严禁复用最近 10 轮内已经使用过的完全相同主题、展现形式或近似视觉观感；不得自行回到近期模板。
7. ${tarotRequirement}
8. 【Toto 边界与水印】<toto> 只作为插件识别边界，不得作为可见 UI；Toto 仅作为插件设置界面的界面水印存在，不得在兔子洞小剧场内部生成 Toto 水印。
9. 如启用 <thinking>，其中只输出可见的执行摘要，不输出隐藏思维链或详细推理过程。
10. 最终答案必须以 <toto data-rabbit-hole="true" style="display:block;"> 开始，并以 </toto> 结束。
</RabbitHoleTheaterSubApiInjection>
`;
