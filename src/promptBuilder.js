...
UI自查与去模板化:
  ...
  rule:
    - "生成前必须回看并执行《格式与美感规范》《展现形式优先原则》与《状态栏隔离》"
    - "本轮小剧场必须像一个独立完成的微型 HTML 作品，而不是正文附录、普通说明卡、状态栏或报告页"
    - "必须根据本轮展现形式重新决定内部 HTML 结构、版式、色彩、层级、视觉锚点与阅读节奏；不得把输出结构说明当成固定模板"
    - "不得只通过更换标题、颜色、图标、边框或装饰来伪装新 UI；如果版式结构、信息排列方式、视觉重心、文本密度或阅读节奏与近期结果相近，必须在输出前重构"
    - "UI审查重点只用于自检质量，不得变成可见栏目、固定组件、固定配色或固定版式"
`;
...
5. 所有 HTML 样式使用 inline style；必须执行自适配、文字安全、复杂度硬指标、展现形式优先、状态栏隔离与近期视觉观感去重。
