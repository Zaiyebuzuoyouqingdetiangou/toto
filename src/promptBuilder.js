import { TAROT_IMAGE_RULES } from '../data/raw/tarotImageRules.js?rmv=0.33.44';
import { VISUAL_SCENERY_RULES } from '../data/raw/visualSceneryRules.js?rmv=0.33.44';
import { pickCombination } from './picker.js?rmv=0.33.44';
import { getComboHistory, getRecentRiskFlags, getRecentRiskFlagCounts, getActivePaletteCooldown } from './storage.js?rmv=0.33.44';
import { readSelectedMemoryForPrompt } from './memoryScanner.js?rmv=0.33.44';
...
复杂交互视觉核心:
  ...
  - 需要状态叠层时，优先使用能由内容撑高的 grid 同格叠层、正常流显隐或媒介内部明确可操作的滚动／分页；禁止让两个含长正文的状态以 absolute 叠放在固定画布内。输出前按 360px 手机窄屏自检，每个状态的最后一行必须仍位于所属卡片、画框或页面边界内。
