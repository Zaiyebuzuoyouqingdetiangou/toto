...
2. 禁止使用英文作为主要 UI 标签、状态栏、警告栏、标题装饰、面板名称或系统提示，例如 SYSTEM_STATUS、WARNING、CRITICAL、ERROR、LOG、REPORT、CHECK、MAX 等。
3. 不得输出全英文界面或大段英文段落；若需要对应 UI 氛围，必须改为“系统状态 / 警告 / 严重 / 错误 / 记录 / 报告 / 检查 / 最大值”等中文。
4. 外语、专有名词、平台名、术语如必须出现，需立即用 [] 给出简体中文翻译，并且不得让外语成为主要可见文本。
`;
...
const UI_BEAUTY_CONCEPT_COOLDOWN_PROTOCOL = String.raw`
【UI美化构思冷却协议】
...
G. UI美化构思：根据本轮展现形式定制高完成度版式，不复用单一模板；本轮UI美化构思：${formatUiBeautyConcept(combo)}；若启用 Visual Scenery，则以纯视觉氛围场景为核心。
${directiveLine}
</thinking>
`;
}
...
【固定注入：UI美化构思冷却协议】
${UI_BEAUTY_CONCEPT_COOLDOWN_PROTOCOL}
...
3. 兔子洞内所有可见文字必须且只能使用简体中文，UI 标签、状态栏、标题、表头、按钮、系统提示也必须中文化；禁止英文承担主要界面标签。外语/术语出现时必须立即用 [] 给出简体中文翻译。
...
7. 必须执行 UI 自查优化、UI 核心驱动协议与 UI美化构思冷却协议：至少满足复杂度 7 项指标中的 4 项，并根据本轮展现形式建立清晰视觉分区、文本层级与专属视觉锚点；避免连续复用同一套外观。
8. 最终小剧场必须体现本轮主题与展现形式的专属设计，不要只输出无版式设计的纯文字堆叠。
9. 严禁用 <br> 制造间距，段落使用 p 标签和 margin/line-height。
10. 严禁复用最近 10 轮内已经使用过的完全相同主题具体子项、展现形式具体子项或 UI美化构思；插件已执行 10 轮冷却，并已列出近期 UI美化构思，模型不得自行回到近期模板。
