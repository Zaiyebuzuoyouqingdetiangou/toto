...
import { UI_AUDIT_RULES } from '../data/raw/uiAuditRules.js';
...
【结尾保底规则】
1. 本轮回复的最后一个可见模块必须是完整的 <toto data-rabbit-hole="true" style="display:block;"> 包裹的 <details> 小剧场。
2. 正文完成后，必须在消息最底部追加该模块；模块必须以 <toto 开始，并以 </toto> 结束。
`;
...
export function buildRabbitHolePrompt(settings, generationType = 'normal') {
    ...
    const uiAuditText = settings.uiAudit ? UI_AUDIT_RULES : '';
...
2. 小剧场最外层必须完整包裹在 <toto data-rabbit-hole="true" style="display:block;"> 与 </toto> 之间，禁止遗漏闭合标签；<toto> 内部必须使用 <details> 折叠模块，并用 <summary> 显示【兔子洞：本次标题】。
3. 兔子洞内所有可见文字必须使用简体中文；禁止英文承担主要界面标签。如必须使用外语或专业术语，必须采用“外语【简体中文翻译】”格式。
4. 内部 HTML 不提供固定模板；必须首先落实本轮展现形式，并通过 UI审查重点与视觉家族硬门槛。经典模式还必须自然融合本轮主题元素；仅展现形式模式不得自行补造主题元素。
5. 所有 HTML 样式使用 inline style；必须执行自适配、文字安全、展现形式优先、状态栏隔离、视觉家族与通用骨架硬门槛。
6. 严禁复用最近 10 轮内已经使用过的完全相同主题、展现形式或近似视觉观感；不得自行回到近期模板。
7. ${tarotRequirement}
8. 【Toto 边界与水印】<toto> 只作为插件识别边界，不得作为可见 UI；Toto 仅作为插件设置界面的界面水印存在，不得在主回复正文或兔子洞小剧场内部生成 Toto 水印。
9. 如启用 <thinking>，其中只输出可见的执行摘要，不输出隐藏思维链或详细推理过程。
10. 不要解释你正在遵守规则，不要输出代码块，直接输出最终可渲染 HTML。
11. 【最终结尾检查】本轮回复的最后一个可见模块必须是完整的兔子洞 <toto data-rabbit-hole="true" style="display:block;"><details>...</details></toto> 小剧场。该模块为本轮回复格式的固定组成部分，不得视为可选彩蛋。
</RabbitHoleTheaterAutoInjection>
`;
