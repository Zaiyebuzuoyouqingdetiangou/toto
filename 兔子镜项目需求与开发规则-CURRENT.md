...
## 2026-09-13 当前修订：1.5.48 正式通道分离

本次以保留的 1.5.48 TT 外部母本修复版为基线，正式 cohort 为 `1.5.48-release1`，manifest 恢复正常 index 入口与正式自动更新标记。按用户要求，仅移除 TT 备用设置栏、其“打开设置与诊断”动作和独立小剧场查看器相关运行代码；正常完整设置、诊断、生成、美化、维修、挨打猫及外部母本修复保留。不新增扫描、网络请求或存储迁移，不覆盖 TT 原包，不推送仓库。不把通道隔离说成已解决 TT 晚注册。说明见 `docs/update-1.5.48-release.md`；以下内容为历史记录，冲突时以本段和本次用户授权为准。

## 2026-09-13 当前修订：1.5.48 外部母本修复测试

本次以保留的 1.5.47 为基线，cohort 为 `1.5.48-externalfix1`，沿用 tt-test 通道。功能修改仅限外部摘要及部分点菜缓存：启用/停用可用条目后新操作不沿用过期的随机选择；较长元数据不能挤掉自动摘要正文。旧库只修正符合旧算法的本次发送副本，不改 IDB 原文、人工不同摘要或历史成品。其余运行模块仅同步版本/缓存。

不新增诊断功能、不改变抽签比例/动态视觉优先级/请求预算、不自动重发。未修复 TT late-projection 内嵌，也未增加跨页面库变更同步。完整边界见 `docs/update-1.5.48.md`。以下旧版本范围和测试数字属于历史记录；冲突时以本次用户授权及本段为准。
