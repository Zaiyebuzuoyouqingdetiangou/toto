...
更新：2026-09-06。当前发布包：1.5.23，基于保留的 1.5.22 公开版；manifest/runtime 为 `1.5.23`，主 cache cohort 为 `1.5.23-ui1`。本文是本包唯一 CURRENT；以下旧版本条目属于历史约束。修复仅限抽签记录显示、设置说明/排序、镜面边界及手机布局。生成提示、模型请求、安全边界不变，不推送仓库或修改在线安装。跨设备保存尚未实现，手机热与长延迟仍需实机验收。
