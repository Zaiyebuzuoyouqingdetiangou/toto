...
## 当前版本：一体化低 token + Toto 水印版

这一版不需要在原预设里放 `{{rabbit_hole}}`。插件会在生成前自动临时注入兔子洞规则，并要求模型在**主回复正文最底部**追加 `<details>` 兔子洞模块。

本版重点：

- 兔子洞作为一个整体协议自动运行，不再拆成 Independent / Canon / Raw Policy 给用户手选。
...
- 抽到正文衍生类条目时，自动允许 Canon 分支；其他情况按独立兔子洞执行。
- 每个兔子洞模块内部底部必须加入固定水印：`Toto`。
...
- 启用兔子洞自动注入：总开关。
- 输出 `<wonderland>` 执行摘要：测试时可开，正式使用建议关闭以节省输出空间。
...
本版已在运行注入规则中追加固定水印要求：每轮生成的【兔子洞】模块需要在模块内部底部显示低透明度小字号水印 `Toto`。
...
本版将 `Toto` 作为插件设置界面的 UI 水印显示，不再注入到 `<details>` 小剧场内部，避免污染生成内容或占用输出。
