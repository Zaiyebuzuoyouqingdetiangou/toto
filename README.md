...
本正式仓库用副本以 QualityFix5 / 1.5.4 的实际源码为基线，修复安卓本地酒馆在独立 API 成功结算时镜面随宿主 DOM 替换消失的问题，并扩充主题／展现母本与近期抽取多样性；不改付费请求单次约束、主／独立 API 边界、标签与世界书、Token 口径或 12,000／20,000／32,000 字符上限。

- 独立 API 已成功保存的精确镜面在宿主完成帧、DOM 替换与后续被动同步中继续按原楼层身份恢复；不会因为成功结算再发送第二次请求。
- 母本库与随机抽取池按既有编号和结构落实用户指定的 24 项主题／展现形式：新增 22 项，并强化已有的“视觉小说”和“伪史论／野史考据”落地约束；原有条目、收藏、黑名单及自定义倍率不被覆盖。
- 主题与展现形式按家族规模和近期命中共同平衡，降低大类天然占比与同一家族近期反复；已拉黑项目仍不进入候选，收藏倍率仍保留用户设定。
- 成人向条目被选中时，把只供内部构造使用的虚构角色扮演边界放进 Prompt，不在兔子镜成品中额外输出破坏沉浸的年龄核验、免责声明或跳出角色说明。
- 发布身份更新为 `1.5.5`；核心模块及本轮变化的 structured/raw 数据链统一使用 `1.5-varietyfix1` cache cohort，避免覆盖安装后混用旧抽取池或旧母本。
- 没有新增 `fetch`、`setInterval`、MutationObserver、常驻轮询、事件监听或全聊天扫描。仅完成源码与自动化验证，尚未完成真实 SillyTavern、真实副 API、安卓／PC／iPhone 与真实模型实机验证；GitHub 未写入，原 ZIP 未覆盖。

## QualityFix5 / 1.5.4

本版以 QualityFix4 / 1.5.3 的实际源码为基线，修复多楼层并发所有权与净化后成品质量退化；不修改母本库、候选池、随机权重、收藏／黑名单、主题与配色冷却、主美化 Prompt、维修兔、挨打猫、Touch Theater、标签／世界书、Token 口径或 12,000／20,000／32,000 字符上限。

- 第一条兔子镜仍在请求或等待结算时发送第二条消息，不再把第一条的唯一 flight 当作过期任务取消；同一宿主操作中的被动正文 hash 漂移也不会误杀已在途请求。
- 生成中的宿主 DOM 被酒馆重绘或替换后，只复挂对应楼层的 loading 外壳；请求结算后只排队一次精确被动同步，不扫描全聊天，也不发第二次付费请求。
- 手动“重说”会先取消同一消息仍在途的旧自动／旧 Swipe 请求并取得唯一写回权；旧响应即使晚到也不能覆盖手动成品。Swipe 同样会终止上一 Swipe 的在途请求。
- 页面恢复、BFCache 或同版本 DOM 重挂时，若精确付费请求仍在运行，只恢复它的 loading 外壳，不会先误报失败再跳回成功。
- 成品在真正的安全净化之后再做一次只读质量检查：拦截退化为通用三按钮／标签切页加纵向文字流、多节点只有一个显隐入口，以及简单实色正文明确低对比的结果。频道、档位、分页器等媒介本体需要该交互时保留。
- 未通过质量门槛的半成品不保存、不挂载、不记为兼容成功，也不会自动重发；同一精确正文手动“重说”时只附加一条有界、30 分钟内有效的避错提示，新正文不继承旧提示。净化成品首次挂载复用有界缓存，不重复解析大型 HTML；抽签结果、候选池和权重不变。
- 发布身份更新为 `1.5.4`，核心模块统一使用 `1.5-qualityfix5` cache-bust 闭包，避免覆盖安装后混用 QualityFix4 模块。
- 没有新增 `fetch`、`setInterval`、MutationObserver、常驻轮询、事件监听或全聊天扫描。仅完成源码与自动化验证，尚未完成真实 SillyTavern、真实 Connection Manager／副 API、PC／iPhone 和真实模型实机验证；GitHub 未写入，原 ZIP 未覆盖。

## QualityFix4 / 1.5.3

本版以用户上传的 `兔子镜测试版-1.5.2-QualityFix3(1).zip` 为唯一源码基线，只修复独立 API 成品生命周期误判；不改母本库、随机抽取、候选池、权重、收藏／黑名单、主题与配色冷却、美化 Prompt、维修兔、挨打猫、Touch Theater、标签过滤、世界书、Token 口径或请求上限。

- 同一聊天、消息和 Swipe 的已完成兔子镜，若正文只被酒馆正则、状态栏或其他后置处理改写，不再被误判成新的正文版本，也不会变成“等待正文完成”或失败壳。旧镜只保持可见，不会被写成新 hash 的缓存。
- Swipe、重说、续写仍须有当前宿主操作或更新的精确授权证据；确认属于新正文时，旧镜继续进入原有 stale-source 保护，不会闪回或错误复用。
- 页面隐藏、恢复、BFCache 或 DOM 重挂后，若当前身份已有精确 mounted／聊天持久记录／本地记录，已消费的一次性请求租约不再覆盖 ready 成品，也不会触发第二次付费请求。
- 版本更新为 `1.5.3`，核心模块统一使用 `1.5-qualityfix4` cache-bust 闭包，避免覆盖安装后混用 QualityFix3 模块。
- 没有新增 `fetch`、`setInterval`、MutationObserver、常驻轮询或全聊天扫描；新增检查只针对当前一条消息的精确身份和已有记录。
- 仅完成源码与自动化验证，尚未完成真实 SillyTavern、真实 Connection Manager／副 API、PC／iPhone 和真实模型实机验证；GitHub 未写入，原 ZIP 未覆盖。

## QualityFix2 / 1.5.1

本版以用户上传的 `兔子镜测试版-1.5-QualityFix1-AwaitingOwnerIdentityFix.zip` 为唯一源码基线，保留外框 owner 恢复和旧 Swipe／旧正文身份隔离修复，并补充两项窄修；不改母本库、随机抽取、美化 Prompt、配色／结构冷却、维修兔、挨打猫、Touch Theater、SSE／NDJSON、DeepSeek 或单次付费请求保护。

- 延迟加载忙碌检测同时识别官方 SillyTavern `#mes_stop` 与旧／魔改宿主 `#stop_but`，并按真实可见性判断；隐藏按钮不会让运行时永久等待，可见按钮也不会被误判为空闲。没有新增 Observer、轮询、定时器或全聊天扫描。
- 标签选择继续供独立 API 对临时上下文副本做真实删除。新增默认关闭的“跟随当前 API 标签隔离”：开启后只在兔子镜近输出规则中禁止 `<toto>` 参考、复述或仿写所选标签块；正文与主预设不删除、不改写，不新增网络请求。由于跟随模式与正文共用同一次主请求，这不是传输层删除。
- 发布身份更新为 `1.5.1`，核心模块统一使用 `1.5-qualityfix2` cache-bust 闭包，避免覆盖安装后继续执行旧 QualityFix1 模块。
- 仅完成源码与自动化验证，尚未完成真实 SillyTavern 1.13～1.18、旧云酒馆、真实主／副 API、真实模型和手机实机验证；GitHub 未写入。
...
本候选在 ModelSelectFix1 上修复正文结束后的自动生成结算、长流响应、失败重说状态与长聊天读取，并增强近期主题／形式的多样性。此前双来源模型选择、简化设置 UI、维修兔目标隔离和问题 emoji 均保留。

- 主生成按当前聊天、消息、正文指纹与工具递归阶段结算；quiet／dry-run 不再误取消已有副 API 请求或冒充正文结束，历史重绘和工具结果系统消息不能触发付费生成。
- 复用 SillyTavern 的同步工具能力检查。工具关闭时，即使 quiet 已消耗唯一一次结束事件，也可从当前准确的正文渲染记录恢复；流式工具中间帧则同步拒绝。
- 对真正有工具歧义的非流式正文，宿主没有提供可靠最终证明时不猜测：主生成不再报告活动后等待最多 12 秒，即显示带 ⚠️ 的未发送请求说明和手动重生入口。仍报告活动的宿主等待保留 10 分钟上限。停止后迟到的工具递归不能自动重开；新用户生成或聊天切换可解除停止标记。
- 独立 API 请求按同一个响应的实际进度续期：连续 5 分钟没有新响应会失败，持续有进度的长流不会在第 5 分钟被强制截断，单次请求仍有 20 分钟绝对上限。没有自动重发或换模型。
- 流式失败只为下一次明确的手动重说准备同参数 `stream=false`；该选择消费一次、20 分钟过期且隔离模型／运行版本。非流式失败不会永久黏住；语义完整的非流式成功才记入兼容配置，6 小时后可重新探测流式。
- 识别嵌套错误中的 Abort／连接中断，避免把中断误报成未知 HTTP 状态；200 响应若成品超复杂度预算，也会在诊断中明确记为失败。
- 每次上下文读取只索引一次当前挂载楼层，不逐条探测未加载的长历史；当前目标正文仍重新读取，不使用历史缓存。系统／工具结果不当成角色正文，不进入最近正文或尾层选题指令。
- 从既有最近 10 次历史计算重复主题组／形式家族的软降权；没有历史时的抽样、收藏倍率、黑名单、精确 ID 避让、候选池和配色冷却保持不变。随机仍可能重复，不保证模型每轮输出不同造型。
- 不新增常驻轮询、Observer 或网络请求。新增等待只绑定当前生成 owner，结束／停止／切聊天时清理；停止记录最多 8 条，仅含聊天标识和时间，没有正文缓存。
- 23 个母本数据文件、规则 Prompt、维修／交互净化、Touch Theater、挨打猫、Token 统计与 12,000／20,000／32,000 字符预算未改。实际出站内容会因当前正文过滤和随机结果改变，不能把整份请求说成逐字不变。

仅完成源码／自动化验证，尚未真机验证。没有把截图中的 AbortError 断言为世界书或网页卡顿的唯一原因；真实 SillyTavern、iPhone／Safari、实际中转 API 的流式与非流式还需安装后验收。GitHub 未写入，原 ZIP 未覆盖。
...
本候选建立在已验证的 AdvancedUI1 Stability1 RepairEmoji1 测试源码上，只调整兔子镜设置主面板的布局和样式；生成、请求、Prompt、维修、保存及高级页面逻辑不变。

- “高级设置”总入口移动到顶部自动注入开关旁，仍是原来的唯一按钮和点击绑定；独立 API 的读取内容快捷入口继续保留。
- Token 总数改为轻量摘要，详细统计按需展开；所有原统计字段和更新逻辑不变。
- 主分区默认收起并改成更紧凑的主题自适应卡片，去除固定米色与重复的底部高级卡片；窄屏自动换行，不增加动画、Observer、定时器、网络请求或聊天扫描。
