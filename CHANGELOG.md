...
- 修复部分手机 SillyTavern 美化对 `body` / 聊天祖先设置 transform、overflow 或裁切后，“兔子镜历史”固定浮层无法正常展开、只在屏幕最上端留下细黑线的问题。
- 支持的浏览器改用原生 `<dialog>.showModal()` 顶层显示历史，不再依赖普通 `position: fixed` 浮层穿过主题的层叠/变换上下文；旧环境保留固定浮层回退。
- 历史预览区域增加独立布局/绘制 containment，历史兔子镜内部若带 fixed 装饰、扫描线或高层级元素，也限制在预览区域内，不再逃逸到页面顶部。
- 关闭历史时会先关闭 dialog 顶层再移除节点，避免 Safari / WebView 留下残余顶层状态。
- 未修改历史数据、兔子镜生成、Prompt、随机抽取、外置宽度、外置一体化、独立 API 请求或 inline/外置后内嵌逻辑；本构建发布通道为正式仓库 `toto`。

# 1.3.11 — 外置宽度与美化容器解耦

- 外置兔子镜不再读取 `.mes_text` / `.mes_block` 的实际宽度或 padding，避免更换 SillyTavern 美化后因正文容器结构不同而再次贴边或忽宽忽窄。
- 纯外置与跟随当前 API 的外置都改为自身响应式 gutter：整个兔子镜本体（标题栏、边框、内容区一起）居中，左右默认各保留约 12px；同时受 `100%`、`100vw` 与桌面 760px 上限共同约束。
- 清理 1.3.10 的正文 content-box 测量与宽度模式覆盖；热更新时会主动清掉旧的几何 CSS 变量，避免旧测量残留。
- `inline` / 外置后内嵌完成后的容器宽度逻辑保持原样，仍为 100% 跟随正文容器，不额外增加这层外置 gutter。
