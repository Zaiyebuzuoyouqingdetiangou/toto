...
- 正式版性能热修：维修兔与挨打猫的聊天区观察器改为按消息增量处理，不再因任意插件 UI 的 class/style 变化全量扫描聊天。
- 忽略 RabbitMirror 自己的工具入口 DOM 变化，增加 120ms 去抖和重入隔离。
- 保存并在停用/热更新时解除全部宿主事件监听，新增全局 cleanup，防止多版本监听叠加。
- 不包含独立 API、外置兔子镜、自动维修或其他测试功能。

# RabbitMirror 1.1.0-beta.14.1.1
