# 1.6.16-test.3 · 等待正文稳定时不再被缺壳补卡打断

- 修复正文结束并获自动生成授权后，又被宿主收尾改写时，缺壳恢复先写入失败停止标记，导致自动生成不启动、手动重说才可生成的问题。
- 当前楼层仍有未取消的生成等待任务时保留 loading；普通同步与重新挂载都不再提前写入缺壳失败。实际发送仍经过原有身份、正文稳定性、失败状态和一次请求检查。
- 稳定失败卡纳入已有被动恢复缓存，减少每次滚动、点击、聚焦都重复同步的情况。正文、状态、身份、重试或反馈节点变化仍触发检查，loading 不作为稳定卡缓存。
- 相关回归 31 项通过；全套 390 项，355 通过、28 项既有失败、7 跳过，失败名单未增加。真实手机锁死未完成验收。版本、缓存一并更新，完整刷新后生效，详见 docs/automatic-preparation-recovery.md。

# 1.6.16-test.2 · 旧登记不再占用独立 API 的任务位置

- 独立 API 的临时计划改由当前页面持有，成功、失败、取消、超时及重试继续按 test.1 释放；刷新或关闭页面后不再留下永久占位。旧共享表满 8 项或损坏不会阻断新的独立请求。
- 抽签尝试记录、公平性记账和成功历史仍持久保存；记账失败仍阻止发送，提交失败保留当前登记以便清理。不删除已保存成品或强制清空旧表，跟随正文继续原路径。
- 保留原有 8 项活动登记与单计划大小保护，不新增限制或付费请求。更新后完整刷新。新增 10 项隔离与实际请求入口回归，完整验证与未验收范围见 docs/batch-registry-release.md。

# 1.6.16-test.1 · 任务登记释放修复（历史记录）

- 修复多面失败或部分成功后，自动补单面／多面时遗漏上一轮登记，累计触发 BATCH_REGISTRY_CAPACITY 的问题。
- 每轮在成功提交之后或失败、重试之前释放自己的登记；释放写入失败时保留身份，在后续清理边界再次尝试。
- 主动取消时，即使宿主适配器不响应 AbortSignal，也能结束本地等待并执行清理。迟到的旧轮回调不能覆盖新轮计划、抽取记录或延长其等待。
- 历史遗留仍按原有 12 小时过期规则回收，不推断其他标签页或跟随模式的任务已死亡，不清空成品、母本或其他插件数据。验证与局限见 docs/batch-registry-release.md。

# 1.6.7 · 重说可以纯点菜

- 重说增加「纯点菜」。选 HTML 或长文本，再写下想要的界面、字数或其他要求。这一次不抽签，也不沿用上一轮选题。
- 原选题重写和重新抽一张仍在。纯点菜的要求不填、或没选形式时，不会发送。

# 1.6.6 · 长文本续写与跟随正文

- 副 API 的最大输出上限从 32000 提到 64000。默认仍是 30000，可以自己改，最多 64000。跟随正文用的是聊天模型自己的输出上限，不受这项限制。
- 呈现方式只留自动、HTML 和长文本。去掉「文本」和「3000–5000 字」。以前选过「文本」的设置会按长文本处理。
- 已经写成故事的面也会出现续写，不再只认新标成「长文本」的配方。HTML 面没有续写。
- 跟随正文的长文本必须把故事写进外壳里的 article。篇幅不够时收短主回复，不能只留标题。已经生成的空壳没有正文可恢复，需要重新生成。

# 1.6.5 · 母本库管理留在导入页

- 「导入小剧场世界书」这一页直接显示母本库的启用、比例和条目勾选。点「管理母本库」只回到这一页，不再另开一层窗口。
- 粘贴文字、导入文件和换设备仍会打开原来的导入窗口。

# 1.6.4-longtext4 · 长文本跟抽什么走

- 去掉「长文本素材来源」。长文本和 HTML 抽同一池，跟「抽什么」走。正文不要写成 HTML；条目里的按钮和交互只当题材。已经生成的空白长文本，重说仍按原来的配方。
- 「抽什么」下面可以打开管理母本库，并写明内置、母本库和世界书的区别。母本库里的比例只决定自带条目和已启用母本怎么分。
- 文风挪到这一页，可选，不限字数。留空不追加。

# 1.6.4-api.11 · 显示设置入口与工具恢复

- 将原切页控件真正移出新版设置的隐藏源容器，放到“设置首页 → 兔子镜显示模式 → 多面切页位置”，可选择顶部／底部切页，原保存和立即生效行为保留。
- 修复工具栏子按钮被宿主重绘后仍复用旧绑定的问题；单面外置镜面替换时重新绑定工具到当前镜面。关闭生图窗口后，对仍在页面上的当前楼层恢复工具，不重生成正文或图片。
- 本地浏览器已验证完整设置路径，以及模拟构思／生图失败、工具节点重绘、关闭窗口、重新打开兔子菜单的流程。没有调用真实模型；用户实际宿主中此次失败的触发链仍待更新后复测。

# 1.6.4-api.10 · 仅文本多面与反馈键盘适配

- 仅有文本条目时，自动／长文本常规抽取不再将主题和展现形式作为前提，也不再按展现形式数量让前面提前耗尽文本池。每面使用一条完整文本，优先不同条目，耗尽后循环复用仍启用的条目；禁用、未确认和明确排除的条目仍不可抽。空白长文本保持空白配方；普通主题＋形式组合沿用原去重规则。
- 修正挨打猫输入反馈时，键盘上方的面板被旧聊天输入框位置压成细条的问题。编辑时使用可见视口并保持输入框可见；空间不足时面板内滚动，关闭键盘后恢复布局。
- 新增 18 项针对性回归；本地浏览器检查分屏、模拟键盘与反馈输入／滚动提交。未调用真实生成服务，真实 iPad 键盘尚待实机验收。更新后完整刷新。

# 1.6.4-api.9 · 切页位置入口移到设置首页

- 将切页位置直接放到兔子镜设置首页，名称改为“多面切页位置”，无需进入高级设置。保留已保存的顶部／底部选择与原有切页行为。

# 1.6.4-api.8 · 顶部／底部切页

- 设置 → 高级设置 → 生成与抽取中新增“切页位置”：顶部切页／底部切页，默认沿用顶部。
- 底部按钮放在本面 details 外框内、内容之后，手机居中、电脑靠右，折叠时隐藏；收藏、兔子和删除保留在标题栏。位置选择立即作用于已挂载控件，原切页和边界重说行为不变。
- 保存、恢复、复制、收藏及生图正文提取剥离运行时底部控件，不改原正文与互动状态。
- 新增 8 项回归通过；全套 282 项中 267 通过、8 项既有失败、7 跳过。使用实际控件函数、样例动作桥接及正式样式完成 360 / 1200 像素浏览器检查，未调用真实生成服务；生产宿主实机尚未验收。
- 更新后完整刷新，再选择“底部切页”。

# 1.6.4-api.7 · 空白长文本跳过无关外置库检查

- 所有面均为空白长文本时，不再因启用了外置库而读取外置数据库或校验旧索引，避免无关库阻止请求。
- 混合 HTML 面、常规抽取、文本来源和按原外部配方重说仍保留材料检查。旧镜面重说沿用原配方和呈现模式，不自动改用新设置。
- 新增 5 项真实请求函数至模拟发送边界的回归测试；不涉及真实服务商调用。顶部／底部切页本版尚未实现，位置仍在预览阶段。

# 1.6.4-api.6 · 生图形式演绎按角色场景理解

- 修正“形式落地”的含义：信件画 char 正在写信，相册画角色翻看相册，将形式转化为角色参与其中的场景；长文本继续选正文高光。
- 同步生图面板及设置说明。已保存的提示词和图片保留，选择“按展现形式演绎／长文本高光”后重新构思生效。

# 1.6.4-api.5 · 长文本、文风与形式生图

- 新增默认 3000–5000 字的长文本面：空白创作／常规抽取／文本类三种素材来源，原条目字数和 HTML 要求优先。
- 创作补充词下增加文风输入；独立 API 可自动参考角色主世界书的已启用背景条目。
- 生图可按展现形式构图、按长文选择高光；保留跨刷新与重说的形式记录，切换构图不清空草稿。
- 新增“印象之匣”和“画给你的自由插画”，采用模型自由构图的安全 HTML/CSS/SVG 绘画。
- 外置库显示当前可抽分类数量，明确文本模式的候选范围。
- 使用方式、验证和边界见 [创作模式说明](docs/creation-modes-api5.md)。更新后完整刷新。
