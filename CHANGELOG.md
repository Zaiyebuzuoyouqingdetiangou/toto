...
- 仅 TT：在 DOM 就绪后立即初始化核心，不再等待 idle、稳定三秒或已挂载聊天。修正 managed 登记失败时核心和正常设置一直等待的问题。
- 入口保留零静态依赖、同步尝试登记及原生成拦截器；移除备用设置栏、两个备用按钮和独立查看面板。
- 云酒馆维持原延迟路径。外部母本、Prompt、维修、成品、一次请求限制等运行实现不变。
- 已关闭的 TT 登记窗口未被重开；late-projection 仍不能在虚化正文原位置内嵌。核心负荷会提前，不承诺真机不卡顿。仅源码/自动化验证，尚未真机验证。
